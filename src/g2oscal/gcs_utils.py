...
def read_json_from_gcs(gcs_path):
    ...
    try:
        logger.debug(f"Reading JSON from gs://{BUCKET_NAME}/{gcs_path}...")
...
def write_json_to_gcs(gcs_path, data):
    ...
    logger.debug(f"Writing JSON to gs://{BUCKET_NAME}/{gcs_path}")
    blob = bucket.blob(gcs_path)
    try:
        json_data = json.dumps(data, indent=2, ensure_ascii=False)
        blob.upload_from_string(json_data, content_type="application/json")
        logger.debug(f"Successfully wrote to gs://{BUCKET_NAME}/{gcs_path}")
    except Exception as e:
